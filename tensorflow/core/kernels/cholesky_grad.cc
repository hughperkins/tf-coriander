...
class CholeskyGrad : public LinearAlgebraOp<Scalar> {
 public:
  ...
  void ComputeMatrix(OpKernelContext* context, const ConstMatrixMaps& inputs,
    ...
    const Eigen::DenseIndex kMatrixSize = input_matrix_l.rows();
    const Eigen::DenseIndex kMaxBlockSize = 32;

    for (Eigen::DenseIndex block_end = kMatrixSize; block_end > 0;
      ...
      */
      ...
      const Eigen::DenseIndex block_begin = std::max(0ll, block_end - kMaxBlockSize);
      const Eigen::DenseIndex block_size = block_end - block_begin;
      const Eigen::DenseIndex trailing_size = kMatrixSize - block_end;
 ...
 private:
  void CholeskyGradUnblocked(const ConstRef& l_block, Ref grad_block) {
    const Eigen::DenseIndex kMatrixSize = l_block.rows();
    for (Eigen::DenseIndex k = kMatrixSize - 1; k >= 0; k--) {
      /* This shows the block structure.
      ...
      */
      ...
      const Eigen::DenseIndex number_rows_B = kMatrixSize - (k + 1);
      const Eigen::DenseIndex number_rows_r_stack_B = number_rows_B + 1;
