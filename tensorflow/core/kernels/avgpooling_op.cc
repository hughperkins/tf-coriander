...
class AvgPoolingGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Eigen::DenseIndex out_backprop_batch = out_backprop.dim_size(0);
    const Eigen::DenseIndex out_backprop_rows = out_backprop.dim_size(1);
    const Eigen::DenseIndex out_backprop_cols = out_backprop.dim_size(2);
    const Eigen::DenseIndex out_backprop_depth = out_backprop.dim_size(3);
    ...
    const Eigen::DenseIndex in_rows = output_shape.dim_size(1);
    const Eigen::DenseIndex in_cols = output_shape.dim_size(2);
    ...
    Eigen::DenseIndex out_height, out_width, pad_rows, pad_cols;
    OP_REQUIRES_OK(context,
    ...
    auto shard = [context, out_backprop_ptr, input_backprop_ptr,
                  ...
                  col_stride, pad_rows, pad_cols](Eigen::DenseIndex start, Eigen::DenseIndex limit) {
      for (Eigen::DenseIndex b = start; b < limit; ++b) {
        for (Eigen::DenseIndex r = 0; r < out_backprop_rows; ++r) {
          ...
          for (Eigen::DenseIndex c = 0; c < out_backprop_cols; ++c) {
            ...
            Eigen::DenseIndex output_index =
                (b * out_backprop_rows + r) * out_backprop_cols + c;
            for (Eigen::DenseIndex r_dst = rindex; r_dst < rindex + rsize; ++r_dst) {
              for (Eigen::DenseIndex c_dst = cindex; c_dst < cindex + csize; ++c_dst) {
                Eigen::DenseIndex input_index = (b * in_rows + r_dst) * in_cols + c_dst;
                const T* output_offset =
                    out_backprop_ptr + output_index * out_backprop_depth;
                T* input_offset =
                    input_backprop_ptr + input_index * out_backprop_depth;
                for (Eigen::DenseIndex d = 0; d < out_backprop_depth; ++d) {
    ...
    const Eigen::DenseIndex shard_cost =
