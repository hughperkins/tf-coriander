...
#define EXTRACT_AND_VERIFY_DIMENSIONS(label)                                   \
  ...
  const Eigen::DenseIndex batch = input_shape.dim_size(0);                                 \
  OP_REQUIRES(                                                                 \
      context, batch == out_backprop.dim_size(0),                              \
      errors::InvalidArgument(                                                 \
          label, ": input and out_backprop must have the same batch size"));   \
  const Eigen::DenseIndex input_rows = input_shape.dim_size(1);                            \
  const Eigen::DenseIndex input_cols = input_shape.dim_size(2);                            \
  const Eigen::DenseIndex filter_rows = filter_shape.dim_size(0);                          \
  const Eigen::DenseIndex filter_cols = filter_shape.dim_size(1);                          \
  const Eigen::DenseIndex output_rows = out_backprop.dim_size(1);                          \
  const Eigen::DenseIndex output_cols = out_backprop.dim_size(2);                          \
  const Eigen::DenseIndex in_depth = input_shape.dim_size(3);                              \
  OP_REQUIRES(context, in_depth == filter_shape.dim_size(2),                   \
              errors::InvalidArgument(                                         \
                  label, ": input and filter must have the same in_depth"));   \
  const Eigen::DenseIndex depth_multiplier = filter_shape.dim_size(3);                     \
...
static void CopyOutputBackpropRegion(const DepthwiseArgs& args,
                                     ...
                                     const Eigen::DenseIndex in_r, const Eigen::DenseIndex in_c,
                                     const T* out_backprop, T* buffer) {
  typedef typename Eigen::internal::packet_traits<T>::type Packet;
  static const Eigen::DenseIndex kPacketSize = (sizeof(Packet) / sizeof(T));

  const Eigen::DenseIndex stride = args.stride;
  const Eigen::DenseIndex filter_rows = args.filter_rows;
  const Eigen::DenseIndex filter_cols = args.filter_cols;
  const Eigen::DenseIndex pad_rows = args.pad_rows;
  const Eigen::DenseIndex pad_cols = args.pad_cols;
  const Eigen::DenseIndex out_rows = args.out_rows;
  const Eigen::DenseIndex out_cols = args.out_cols;

  // Calculate the output spatial region which used point (in_r, in_c) as input.
  const Eigen::DenseIndex out_r_start = std::max(
      static_cast<Eigen::DenseIndex>(0), (in_r - filter_rows + pad_rows + stride) / stride);
  const Eigen::DenseIndex out_r_end = std::min(out_rows - 1, (in_r + pad_rows) / stride);
  const Eigen::DenseIndex out_c_start = std::max(
      static_cast<Eigen::DenseIndex>(0), (in_c - filter_cols + pad_cols + stride) / stride);
  const Eigen::DenseIndex out_c_end = std::min(out_cols - 1, (in_c + pad_cols) / stride);
  ...
  const Eigen::DenseIndex vectorized_size = (args.out_depth / kPacketSize) * kPacketSize;
  const Eigen::DenseIndex scalar_size = args.out_depth % kPacketSize;
  const Eigen::DenseIndex pad_size = scalar_size > 0 ? kPacketSize - scalar_size : 0;

  for (int out_r = out_r_start; out_r <= out_r_end; ++out_r) {
    const Eigen::DenseIndex f_r = in_r + pad_rows - out_r * stride;
    for (int out_c = out_c_start; out_c <= out_c_end; ++out_c) {
      const Eigen::DenseIndex f_c = in_c + pad_cols - out_c * stride;
      const Eigen::DenseIndex buf_base =
      ...
      for (Eigen::DenseIndex d = 0; d < vectorized_size; d += kPacketSize) {
      ...
      for (Eigen::DenseIndex d = 0; d < pad_size; ++d) {
...
static void ComputeBackpropInput(const DepthwiseArgs& args,
  ...
  const Eigen::DenseIndex dm_vectorized_size =
      (depth_multiplier / kPacketSize) * kPacketSize;
  const Eigen::DenseIndex dm_scalar_size = depth_multiplier % kPacketSize;
  ...
  if (depth_multiplier > 1) {
    for (Eigen::DenseIndex d = 0; d < in_depth; ++d) {
      const Eigen::DenseIndex index = d * args.depth_multiplier;
      T accum = 0;
      for (Eigen::DenseIndex dm = 0; dm < dm_vectorized_size; dm += kPacketSize) {
      ...
      for (Eigen::DenseIndex dm = 0; dm < dm_scalar_size; ++dm) {
...
struct LaunchDepthwiseConvBackpropInputOp<CPUDevice, T> {
  ...
  static void launch(OpKernelContext* ctx, const DepthwiseArgs& args,
    ...
    auto shard = [&ctx, &args, &out_backprop, &filter_data, &in_backprop](
        Eigen::DenseIndex start, Eigen::DenseIndex limit) {
      static const Eigen::DenseIndex kPacketSize = (sizeof(Packet) / sizeof(T));
      ...
      for (Eigen::DenseIndex b = start; b < limit; ++b) {
        for (Eigen::DenseIndex in_r = 0; in_r < args.in_rows; ++in_r) {
          for (Eigen::DenseIndex in_c = 0; in_c < args.in_cols; ++in_c) {
    ...
    const Eigen::DenseIndex shard_cost = args.in_rows * args.in_cols * args.out_depth;
...
static void ComputeBackpropFilter(const DepthwiseArgs& args,
                                  const Eigen::DenseIndex padded_out_depth_size,
                                  const Eigen::DenseIndex out_r, const Eigen::DenseIndex out_c,
  ...
  const Eigen::DenseIndex output_image_size =
      args.out_rows * args.out_cols * args.out_depth;
  const Eigen::DenseIndex output_last_vector_index =
  ...
  if (fast_path) {
    ...
    for (int i = 0; i < output_vectorized_size; i += kPacketSize) {
      ...
      for (int j = 0; j < filter_spatial_size; ++j) {
        const Eigen::DenseIndex index = i + j * padded_out_depth_size;
    ...
    for (int i = 0; i < output_vectorized_size; i += kPacketSize) {
      ...
      const Eigen::DenseIndex out_bprop_index = base_output_index + i;
      const Eigen::DenseIndex out_bprop_limit =
          std::min(output_image_size, out_bprop_index + kPacketSize);
      T out_buf[kPacketSize];
      memset(&out_buf, 0, kPacketSize * sizeof(T));
      const Eigen::DenseIndex scalar_size = out_bprop_limit - out_bprop_index;
      for (Eigen::DenseIndex j = 0; j < scalar_size; ++j) {
...
struct LaunchDepthwiseConvBackpropFilterOp<CPUDevice, T> {
  ...
  static void launch(OpKernelContext* ctx, const DepthwiseArgs& args,
    ...
    auto shard = [&ctx, &args, &out_backprop, &input, &output_buffer_data](
      ...
      const Eigen::DenseIndex output_image_size =
          args.out_rows * args.out_cols * args.out_depth;
      const Eigen::DenseIndex padded_filter_size =
    ...
    const Eigen::DenseIndex out_depth = args.out_depth;
    const Eigen::DenseIndex vectorized_size = (out_depth / kPacketSize) * kPacketSize;
    const Eigen::DenseIndex scalar_size = out_depth - vectorized_size;
    const Eigen::DenseIndex padded_filter_size =
        filter_spatial_size * padded_out_depth_size;
    memset(filter_backprop, 0, filter_spatial_size * out_depth * sizeof(T));

    for (Eigen::DenseIndex i = 0; i < filter_spatial_size; ++i) {
      const Eigen::DenseIndex buffer_base = i * padded_out_depth_size;
      const Eigen::DenseIndex output_base = i * out_depth;
      // Write vectorized length of filter's inner dimension to output.
      for (Eigen::DenseIndex j = 0; j < vectorized_size; j += kPacketSize) {
