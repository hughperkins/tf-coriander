              ...
              for (int in_channel = 0; in_channel < input_depth; ++in_channel) {
                ...
                if ((conv_in_x >= 0) && (conv_in_x < padded_width) &&
                  ...
                  if (SampleMode == NEAREST) {
                    const T1 top_left(input_data((Eigen::DenseIndex)batch, (Eigen::DenseIndex)top_y_index,
                                                 (Eigen::DenseIndex)left_x_index, (Eigen::DenseIndex)in_channel));
                    in_value = top_left;
                  } else if (SampleMode == BILINEAR) {
                    const T1 top_left(input_data((Eigen::DenseIndex)batch, (Eigen::DenseIndex)top_y_index,
                                                 (Eigen::DenseIndex)left_x_index, (Eigen::DenseIndex)in_channel));
                    const T1 top_right(input_data((Eigen::DenseIndex)batch, (Eigen::DenseIndex)top_y_index,
                                                  (Eigen::DenseIndex)right_x_index, (Eigen::DenseIndex)in_channel));
                    const T1 bottom_left(input_data((Eigen::DenseIndex)batch, (Eigen::DenseIndex)bottom_y_index,
                                                    (Eigen::DenseIndex)left_x_index, (Eigen::DenseIndex)in_channel));
                    const T1 bottom_right(input_data(
                        (Eigen::DenseIndex)batch, (Eigen::DenseIndex)bottom_y_index, (Eigen::DenseIndex)right_x_index, (Eigen::DenseIndex)in_channel));
