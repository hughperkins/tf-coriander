...
class FractionalMaxPoolOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    std::vector<Eigen::DenseIndex> height_cum_seq;
    std::vector<Eigen::DenseIndex> width_cum_seq;
    if (deterministic_) {
    ...
    OP_REQUIRES_OK(
        context,
        context->allocate_output(
            1, TensorShape({static_cast<Eigen::DenseIndex>(height_cum_seq.size())}),
            &output_height_seq_tensor));
    Tensor* output_width_seq_tensor = nullptr;
    OP_REQUIRES_OK(
        context, context->allocate_output(
                     2, TensorShape({static_cast<Eigen::DenseIndex>(width_cum_seq.size())}),
    ...
    auto output_height_seq_flat = output_height_seq_tensor->flat<Eigen::DenseIndex>();
    auto output_width_seq_flat = output_width_seq_tensor->flat<Eigen::DenseIndex>();
    ...
    // 2: width / col
    // 3: depth / channel
    const Eigen::DenseIndex height_max = input_size_[1] - 1;
    const Eigen::DenseIndex width_max = input_size_[2] - 1;
    for (Eigen::DenseIndex b = 0; b < input_size_[0]; ++b) {
      // height sequence.
      for (Eigen::DenseIndex hs = 0; hs < height_cum_seq.size() - 1; ++hs) {
        // height start and end.
        const Eigen::DenseIndex height_start = height_cum_seq[hs];
        Eigen::DenseIndex height_end =
            overlapping_ ? height_cum_seq[hs + 1] : height_cum_seq[hs + 1] - 1;
        height_end = std::min(height_end, height_max);

        // width sequence.
        for (Eigen::DenseIndex ws = 0; ws < width_cum_seq.size() - 1; ++ws) {
          const Eigen::DenseIndex out_offset =
              (b * output_size_[1] + hs) * output_size_[2] + ws;
          // width start and end.
          const Eigen::DenseIndex width_start = width_cum_seq[ws];
          Eigen::DenseIndex width_end =
              overlapping_ ? width_cum_seq[ws + 1] : width_cum_seq[ws + 1] - 1;
          width_end = std::min(width_end, width_max);
          for (Eigen::DenseIndex h = height_start; h <= height_end; ++h) {
            for (Eigen::DenseIndex w = width_start; w <= width_end; ++w) {
 ...
 private:
  ...
  mutex mu_;
  std::vector<Eigen::DenseIndex> height_cum_seq_;
  std::vector<Eigen::DenseIndex> width_cum_seq_;
...
REGISTER_FRACTIONALMAXPOOL(Eigen::DenseIndex);
...
class FractionalMaxPoolGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    typedef Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>>
        EigenMatrixMap;
    typedef Eigen::Map<Eigen::Matrix<Eigen::DenseIndex, Eigen::Dynamic, Eigen::Dynamic>>
        EigenIndexMatrixMap;
    ...
    constexpr int tensor_in_and_out_dims = 4;
    std::vector<Eigen::DenseIndex> input_size;
    std::vector<Eigen::DenseIndex> output_size;
    for (int i = 0; i < tensor_in_and_out_dims; ++i) {
    ...
    OP_REQUIRES_OK(context, context->allocate_temp(DataTypeToEnum<Eigen::DenseIndex>::v(),
                                                   tensor_out.shape(),
                                                   &tensor_out_arg_max));
    ...
    EigenIndexMatrixMap tensor_out_arg_max_mat(
        tensor_out_arg_max.flat<Eigen::DenseIndex>().data(), output_size[3],
        output_size[2] * output_size[1] * output_size[0]);

    tensor_out_arg_max.flat<Eigen::DenseIndex>().setConstant(kInvalidMaxPoolingIndex);
    // Initializes the duplicate output tensor with MIN<T>.
    tensor_out_dup.flat<T>().setConstant(Eigen::NumTraits<T>::lowest());

    auto height_seq_tensor_flat = height_seq_tensor.flat<Eigen::DenseIndex>();
    auto width_seq_tensor_flat = width_seq_tensor.flat<Eigen::DenseIndex>();
    ...
    // 2: width / col
    // 3: depth / channel
    const Eigen::DenseIndex height_max = input_size[1] - 1;
    const Eigen::DenseIndex width_max = input_size[2] - 1;
    for (Eigen::DenseIndex b = 0; b < input_size[0]; ++b) {
      // height sequence.
      for (Eigen::DenseIndex hs = 0; hs < height_seq_tensor.dim_size(0) - 1; ++hs) {
        // height start and end.
        const Eigen::DenseIndex height_start = height_seq_tensor_flat(hs);
        Eigen::DenseIndex height_end = overlapping_ ? height_seq_tensor_flat(hs + 1)
                                        : height_seq_tensor_flat(hs + 1) - 1;
        height_end = std::min(height_end, height_max);

        // width sequence.
        for (Eigen::DenseIndex ws = 0; ws < width_seq_tensor.dim_size(0) - 1; ++ws) {
          const Eigen::DenseIndex out_index =
              (b * output_size[1] + hs) * output_size[2] + ws;
          // width start and end.
          const Eigen::DenseIndex width_start = width_seq_tensor_flat(ws);
          Eigen::DenseIndex width_end = overlapping_ ? width_seq_tensor_flat(ws + 1)
    ...
    ConstEigenMatrixMap tensor_out_mat(
        ...
        output_size[2] * output_size[1] * output_size[0]);
    const Eigen::DenseIndex num_reshaped_cols =
        output_size[2] * output_size[1] * output_size[0];
    for (Eigen::DenseIndex i = 0; i < num_reshaped_cols; ++i) {
      for (Eigen::DenseIndex j = 0; j < output_size[3]; ++j) {
    ...
    auto out_arg_max_flat = tensor_out_arg_max.flat<Eigen::DenseIndex>();
...
REGISTER_FRACTIONALMAXPOOLGRAD(Eigen::DenseIndex);
