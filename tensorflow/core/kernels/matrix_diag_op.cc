...
class MatrixDiagPartOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Eigen::DenseIndex min_dim = std::min(input_shape.dim_size(rank - 2),
...
class MatrixDiagOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Eigen::DenseIndex k = input_shape.dim_size(rank - 1);
...
struct MatrixDiag<CPUDevice, T> {
  static void Compute(const CPUDevice& d,
    ...
    for (Eigen::DenseIndex r = 0; r < output.dimension(0); ++r) {
      for (Eigen::DenseIndex d = 0; d < output.dimension(1); ++d) {
