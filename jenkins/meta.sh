...
aws s3 cp out.log s3://hughperkins-ci/${PROJECT}/${BUILD_NUMBER}-${GIT_COMMIT_SHORT}.d/${BUILD_NUMBER}-${GIT_COMMIT_SHORT}-build.log
echo "<a href=\"https://s3.amazonaws.com/hughperkins-ci/${PROJECT}/${BUILD_NUMBER}-${GIT_COMMIT_SHORT}.d/${BUILD_NUMBER}-${GIT_COMMIT_SHORT}-build.log\">${BUILD_NUMBER}-${GIT_COMMIT_SHORT}-build.log</a><br />" >> newhtml.html

aws s3 cp artifacts.zip s3://hughperkins-ci/${PROJECT}/${BUILD_NUMBER}-${GIT_COMMIT_SHORT}.d/${BUILD_NUMBER}-${GIT_COMMIT_SHORT}-artifacts.zip
echo "<a href=\"https://s3.amazonaws.com/hughperkins-ci/${PROJECT}/${BUILD_NUMBER}-${GIT_COMMIT_SHORT}.d/${BUILD_NUMBER}-${GIT_COMMIT_SHORT}-artifacts.zip\">${BUILD_NUMBER}-${GIT_COMMIT_SHORT}-artifacts.zip</a><br />" >> newhtml.html
